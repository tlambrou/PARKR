...
<<<<<<< HEAD
    let mapTap = UITapGestureRecognizer(target: self, action: #selector(enlargeMapView(sender:)))
    
    func enlargeMapView(sender: UITapGestureRecognizer) {
        print("Map View is tapped!")
        let vc: AnyObject? = self.storyboard?.instantiateViewController(withIdentifier: "EnlargeMap")
